        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Этаж: {{ $floor->name }}</h3>
                    ...
                    <form method="POST" action="{{ route('admin.floors.update', $floor->id) }}">
                        ...
                        <div>
                            ...
                            <select id="building_id" name="building_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach ($buildings as $building)
                                    <option value="{{ $building->id }}" {{ old('building_id', $floor->building_id) == $building->id ? 'selected' : '' }}>{{ $building->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $floor->name)" required autofocus />
