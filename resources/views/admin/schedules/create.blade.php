    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8">
                <h3 class="text-2xl font-semibold text-gray-900 mb-6">Жаңы расписание түзүү</h3>
                ...
                <form method="POST" action="{{ route('admin.schedules.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="user_id" name="user_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            @foreach ($medics as $medic)
                                <option value="{{ $medic->id }}" {{ old('user_id') == $medic->id ? 'selected' : '' }}>{{ $medic->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="day_of_week" name="day_of_week" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            <option value="Monday" {{ old('day_of_week') === 'Monday' ? 'selected' : '' }}>Дүйшөмбү</option>
                            <option value="Tuesday" {{ old('day_of_week') === 'Tuesday' ? 'selected' : '' }}>Шейшемби</option>
                            <option value="Wednesday" {{ old('day_of_week') === 'Wednesday' ? 'selected' : '' }}>Шаршемби</option>
                            <option value="Thursday" {{ old('day_of_week') === 'Thursday' ? 'selected' : '' }}>Бейшемби</option>
                            <option value="Friday" {{ old('day_of_week') === 'Friday' ? 'selected' : '' }}>Жума</option>
                            <option value="Saturday" {{ old('day_of_week') === 'Saturday' ? 'selected' : '' }}>Ишемби</option>
                            <option value="Sunday" {{ old('day_of_week') === 'Sunday' ? 'selected' : '' }}>Жекшемби</option>
                        </select>
